#include "submarinemovement.h"
#include "gamewindow.h"

// Constructor for submarine -- initialise with some default values
SubmarineMovement::SubmarineMovement()
{
	m_position = glm::vec3(71, 10, 71);
...
// Set the submarine at a specific position, looking at the view point, with a given up vector
void SubmarineMovement::Set(glm::vec3 &position, glm::vec3 &viewpoint, glm::vec3 &upVector)
{
	m_position = position;
	m_view = viewpoint;
	m_upVector = upVector;
...
void SubmarineMovement::SetViewByMouse()
{
...
// Rotate the submarine view point -- this effectively rotates the submarine since it is looking at the view point
void SubmarineMovement::RotateViewPoint(float fAngle, const glm::vec3 &vPoint)
{
...
// Strafe the submarine (side to side motion)
void SubmarineMovement::Strafe(double direction)
{
...
// Advance the submarine (forward / backward motion)
void SubmarineMovement::Advance(double direction)
{
	float speed = (float)(m_speed * direction);
...
// Update the submarine to respond to mouse motion for rotations and keyboard for translation
void SubmarineMovement::Update(double dt)
{
...
// Update the submarine to respond to key presses for translation
void SubmarineMovement::TranslateByKeyboard(double dt)
{
...
glm::vec3 SubmarineMovement::GetPosition() const
{
	return m_position;
}

// Return the submarine view point
glm::vec3 SubmarineMovement::GetView() const
{
	return m_view;
}

// Return the submarine up vector
glm::vec3 SubmarineMovement::GetUpVector() const
{
	return m_upVector;
}

// Return the submarine strafe vector
glm::vec3 SubmarineMovement::GetStrafeVector() const
{
	return m_strafeVector;
}

// Return the submarine perspective projection matrix
glm::mat4* SubmarineMovement::GetPerspectiveProjectionMatrix()
{
	return &m_perspectiveProjectionMatrix;
}

// Return the submarine orthographic projection matrix
glm::mat4* SubmarineMovement::GetOrthographicProjectionMatrix()
{
	return &m_orthographicProjectionMatrix;
}

// Set the submarine perspective projection matrix to produce a view frustum with a specific field of view, aspect ratio, 
// and near / far clipping planes
void SubmarineMovement::SetPerspectiveProjectionMatrix(float fov, float aspectRatio, float nearClippingPlane, float farClippingPlane)
{
...
void SubmarineMovement::SetOrthographicProjectionMatrix(int width, int height)
{
	m_orthographicProjectionMatrix = glm::ortho(0.0f, float(width), 0.0f, float(height));
}

// Get the camera view matrix
glm::mat4 SubmarineMovement::GetViewMatrix()
{
...
glm::mat3 SubmarineMovement::ComputeNormalMatrix(const glm::mat4 &modelViewMatrix)
{
